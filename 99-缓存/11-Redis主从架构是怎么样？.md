...
![1663225311880](D:\OpenCode\mwiki-main\mNote\library\99-缓存\1663225311880.png)
...
- Redis 采用**`异步方式`**复制数据到 slave 节点，不过从 Redis 2.8 开始，`slave node` 会周期性的确认自己每次复制的数据量；
- 一个 `master node` 是可以分配多个 `slave node` 的；
- `slave node` 也可以连接其他的 `slave node`;
- `slave node` 做复制的时候，不会 block master node 的正常工作；
- `slave node` 在做复制时，也不会 block 最自己的查询操作，它会用**旧数据集**来提供服务；但是复制完成时，需要删除旧数据集，加载新数据集，这个时候就会暂停对外的服务；
...
![1663228319356](D:\OpenCode\mwiki-main\mNote\library\99-缓存\1663228319356.png)
...
![1663228896294](D:\OpenCode\mwiki-main\mNote\library\99-缓存\1663228896294.png)
