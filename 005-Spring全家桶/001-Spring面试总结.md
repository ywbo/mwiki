...
- 缓存
- 事务
- 【实际使用】注解 @CheckUploadTimes 
  - 实现方式：为防止过渡频繁的上传文件保证系统正常，我们在上传接口上做限制，每个人每天仅有1000次的上传限制；
  - UploadTimesAspectJ.calss ---- 利用AOP的原理，切面编程
  - 获取到当前登录人，缓存中拿到当前登录人今天操作文件的时间，做一个统计上传次数的操作；
  - 当次数超过1000后，就会报异常。
...
二者非常类似，都像一个嵌套事务，如果不存在一个活动的事务，都会开启一个新的事务。
